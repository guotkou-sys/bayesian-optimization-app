...
// 引入客户端贝叶斯优化库
import { BayesianOptimization } from 'bayes-opt-js';
...
export default function HomePage() {
  ...
  const [optimizer, setOptimizer] = useState<BayesianOptimization | null>(null);
  ...
  const startOptimization = async () => {
    ...
    const integerParams: string[] = [];
    const categoricalParams: { [key: string]: string[] } = {};
    ...
    for (let param of parameters) {
      ...
      if (param.type === 'categorical') {
        ...
        categoricalParams[name] = categories;
      } else {
        ...
        bounds[name] = [parsedRange[0], parsedRange[1]];
        
        if (param.type === 'integer') {
          integerParams.push(name);
        }
      }
    }
    ...
    try {
      ...
      const bayesOpt = new BayesianOptimization({
        ...
        categoricalParameters: categoricalParams,
        bounds: bounds,
      ...
      for (const trial of manualTrials) {
        ...
        bayesOpt.addObservation(
          numericParams,
          parseFloat(trial.value)
        );
      }
      ...
      // 获取下一个建议
      await fetchNextSuggest(bayesOpt, localCategoryMaps);
    } catch (e: any) {
  ...
  const fetchNextSuggest = async (bayesOpt: BayesianOptimization, maps: Record<string, string[]>) => {
    try {
      setIsLoading(true);
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (currentIter >= totalIter) {
        // 优化完成，获取最佳结果
        const bestParams = optimizer.getBestParameters();
        ...
        // 获取下一个建议
        await fetchNextSuggest(optimizer, categoryMaps);
      }
    } catch (e: any) {
  ...
  const resumeOptimization = async () => {
    ...
    try {
      ...
      // 重新创建优化器并添加历史数据
      const bayesOpt = new BayesianOptimization({
        parameters: savedConfig.parameters.map((p: Parameter) => p.name.trim()).filter(Boolean),
        maximize: savedConfig.direction === 'maximize',
        utilityFunction: 'ucb',
        kappa: 2.576,
        xi: 0.0
      });
      ...
      setOptimizer(bayesOpt);
      await fetchNextSuggest(bayesOpt, categoryMaps);
    } catch (e: any) {
